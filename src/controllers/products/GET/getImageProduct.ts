...
export const getImageProduct = async (req: Request, res: Response) => {
  ...
  if (!query.public_id) res.status(400).json({ error: "Falta public_id" });
  if (typeof query.public_id !== "string")
    res.status(400).json({ error: "public_id invalido" });
  const image_url = getSecureUrl(query!.public_id as string, user_id!);
